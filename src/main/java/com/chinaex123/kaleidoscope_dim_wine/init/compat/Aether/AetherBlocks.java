...
public interface AetherBlocks {
    ...
    // -------------------- 酒类 --------------------
    // 酒类
    RegistryObject<Block> DIVINE_OFFERING_BREW = BLOCK_REGISTER.register("divine_offering_brew", () -> DrinkBlock.create().maxCount(2).irregular().shapes(DrinkShapes.getShortBottleShapes()).build().get());
    ...
    RegistryObject<Block> GINGERBREAD_WARMER = BLOCK_REGISTER.register("gingerbread_warmer", () -> DrinkBlock.create().maxCount(2).irregular().shapes(DrinkShapes.getShortBottleShapes()).build().get());
