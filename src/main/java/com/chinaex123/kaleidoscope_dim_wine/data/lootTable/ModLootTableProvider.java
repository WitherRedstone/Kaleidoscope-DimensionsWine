...
public class ModLootTableProvider implements LootTableSubProvider {
    ...
    public void generate(BiConsumer<ResourceLocation, LootTable.Builder> output) {
        ...
        if (ModList.get().isLoaded("twilightforest")) {
            output.accept(ResourceLocation.fromNamespaceAndPath("kaleidoscope_dim_wine", "advancements/twilightforest_wine"),
                    LootTable.lootTable()
        ...
        if (ModList.get().isLoaded("the_bumblezone")) {
            output.accept(ResourceLocation.fromNamespaceAndPath("kaleidoscope_dim_wine", "advancements/mead_essence"),
                    LootTable.lootTable()
                            .withPool(LootPool.lootPool()
                                    .setRolls(ConstantValue.exactly(1))
                                    .add(LootItem.lootTableItem(TheBumblezoneItems.MEAD_ESSENCE.get())) // 蜜酒精华
                            )
            );
        }
    }
